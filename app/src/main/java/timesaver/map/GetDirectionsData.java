...
public class GetDirectionsData extends AsyncTask<Object,String,String> {
    ...
    public void displayDirection(String[] directionsList) throws ExecutionException, InterruptedException {
        ...
        for(int i = 0;i<count;i++)
        {
            ...
            List<LatLng> latList=PolyUtil.decode(directionsList[i]);
            if(latList!=null && i<(count-2)){
                LatLng llng=new LatLng((latList.get(latList.size()-1).latitude+0.003),(latList.get(latList.size()-1).longitude+0.003));
                vect.add(llng);
            }

          //if(i==val){
            //  latll=latList.get(latList.size()-1);
          //}
